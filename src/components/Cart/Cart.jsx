...
function Cart({cart,onhandleUpdateCartQty,onhandleRemoveFromCart,onhandleEmptyCart}) {
     ...
     <Grid container spacing={3}>
        {cart.line_items.map((item)=>(
            <Grid item xs={12} sm={4} key ={item.id}>
              <CartItem item={item} handleUpdateCartQty={onhandleUpdateCartQty} handleRemoveFromCart={onhandleRemoveFromCart}></CartItem>
              </Grid>
              ))}
              </Grid>
